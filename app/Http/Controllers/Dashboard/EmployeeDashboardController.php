...
class EmployeeDashboardController extends Controller
{
    public function index()
    {
        ...
        // FIXED: Use user ID consistently throughout since job_users table stores user_id
        $userId = $user->id;
        
        // Employee-specific statistics
        $stats = [
            'my_active_jobs' => $this->getMyActiveJobs($userId)->count(),
            'my_completed_jobs' => $this->getMyCompletedJobs($userId)->count(),
            'my_pending_tasks' => $this->getMyPendingTasks($userId)->count(),
            'my_in_progress_tasks' => $this->getMyInProgressTasks($userId)->count(),
            'my_completed_tasks' => $this->getMyCompletedTasks($userId)->count(),
            'my_overdue_tasks' => $this->getMyOverdueTasks($userId)->count(),
        ];

        // Performance metrics
        $performanceStats = [
            'tasks_completed_this_week' => $this->getMyCompletedTasks($userId)
                ->whereBetween('updated_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])
                ->count(),
            'tasks_completed_this_month' => $this->getMyCompletedTasks($userId)
                ->whereMonth('updated_at', Carbon::now()->month)
                ->whereYear('updated_at', Carbon::now()->year)
                ->count(),
            'average_task_completion_time' => $this->getAverageTaskCompletionTime($userId),
            'on_time_completion_rate' => $this->getOnTimeCompletionRate($userId),
        ];

        // FIXED: My current active jobs with progress
        $myActiveJobs = $this->getMyActiveJobs($userId)
            ->with(['jobType', 'client', 'equipment'])
            ->orderBy('priority', 'asc')
            ->orderBy('due_date', 'asc')
            ->get()
            ->map(function ($job) use ($userId) {
                $job->my_tasks_count = $this->getMyJobTasks($job->id, $userId)->count();
                $job->my_completed_tasks = $this->getMyJobTasks($job->id, $userId)
        ...
        // FIXED: My current tasks with details
        $myActiveTasks = $this->getMyActiveTasks($userId)
            ->with(['job.jobType', 'job.client', 'jobUsers' => function($query) use ($userId) {
        ...
        // FIXED: My recent completed tasks
        $myRecentCompletedTasks = $this->getMyCompletedTasks($userId)
        ...
        // FIXED: Tasks by status for this employee
        $tasksByStatus = Task::whereHas('jobUsers', function ($query) use ($userId) {
                $query->where('user_id', $userId);
            })
        ...
        // FIXED: Upcoming deadlines for my jobs/tasks
        $upcomingDeadlines = $this->getMyActiveTasks($userId)
            ->with(['job.jobType', 'job.client', 'jobUsers' => function($query) use ($userId) {
                $query->where('user_id', $userId);
            }])
            ->whereHas('job') // Ensure job exists
            ->whereHas('jobUsers', function($query) use ($userId) {
                $query->where('user_id', $userId)
        ...
        // FIXED: My workload over time (last 6 months)
        $workloadTrends = JobUser::where('user_id', $userId)
        ...
        // FIXED: Task completion trends by day for this month
        $dailyCompletionTrends = Task::whereHas('jobUsers', function ($query) use ($userId) {
                $query->where('user_id', $userId);
            })
            ->whereHas('job') // Ensure job exists
    ...
    public function updateTaskStatus(Request $request, Task $task)
    {
        ...
        // FIXED: Use user ID instead of employee ID
    ...
    public function updateJobStatus(Request $request, Job $job)
    {
        ...
        // FIXED: Check if this user is assigned to this job
        $isAssigned = JobUser::where('user_id', $user->id)
    ...
    // FIXED: Helper methods - all using userId consistently
    private function getMyActiveJobs($userId)
    {
        // Get jobs where this user is assigned and the job is active
        // and not completed or cancelled
        return Job::whereHas('jobUsers', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })->where('active', true)
          ->whereNotIn('status', ['completed', 'cancelled']);
    }

    private function getMyCompletedJobs($userId)
    {
        return Job::whereHas('jobUsers', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })->where('status', 'completed');
    ...
    private function getMyActiveTasks($userId)
    {
        return Task::whereHas('jobUsers', function ($query) use ($userId) {
            $query->where('user_id', $userId);
    ...
    private function getMyPendingTasks($userId)
    {
        return Task::whereHas('jobUsers', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })->whereHas('job', function ($query) {
    ...
    private function getMyInProgressTasks($userId)
    {
        return Task::whereHas('jobUsers', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })->whereHas('job', function ($query) {
    ...
    private function getMyCompletedTasks($userId)
    {
        return Task::whereHas('jobUsers', function ($query) use ($userId) {
            $query->where('user_id', $userId);
        })->whereHas('job', function ($query) {
    ...
    private function getMyOverdueTasks($userId)
    {
        return Task::whereHas('jobUsers', function ($query) use ($userId) {
            $query->where('user_id', $userId)
                  ->where('end_date', '<', Carbon::now());
    ...
    private function getMyJobTasks($jobId, $userId)
    {
    ...
    private function getAverageTaskCompletionTime($userId)
    {
        $completedTasks = JobUser::where('user_id', $userId)
    ...
    private function getOnTimeCompletionRate($userId)
    {
        $completedTasks = JobUser::where('user_id', $userId)
            ->where('status', 'completed')
            ->whereNotNull('end_date')
