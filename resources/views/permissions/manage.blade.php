                    ...
                    @foreach ($pageCategories as $tabPageCategory)
                        <li class="nav-item">
                            <button class="nav-link {{ $loop->first ? 'active' : '' }}"
                                id="category-{{ $tabPageCategory->id }}-tab" data-bs-toggle="tab"
                ...
                <div class="tab-content mt-3">
                    @foreach ($pageCategories as $tabPageCategory)
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                            style="background-color: #ffffff;" id="category-{{ $tabPageCategory->id }}" role="tabpanel">

                            <form action="{{ route('admin.permissions.update', ['roleId' => $role->id]) }}" method="POST"
                                                ...
                                                <td>
                                                    <div class="form-check form-switch">
                                                        <input type="checkbox" class="form-check-input permission-toggle"
                                                            ...
                                                            name="permissions[{{ $page->id }}]" value="allow"
        ...
        $(document).ready(function() {
            ...
            function performSearch() {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        $('#search-results').html(
                            '<div class="list-group-item text-danger">Error: orm search</div>');
