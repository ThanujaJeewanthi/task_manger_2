                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.pages.update', $page->id) }}" method="POST">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control @error('code') is-invalid @enderror"
                                        id="code" name="code" value="{{ old('code', $page->code) }}" required>
                                    <div class="input-group-append">
                                        <span class="input-group-text">
                                            <i class="fas fa-info-circle" data-toggle="tooltip"
                                                title="Format: CategoryID.PageNumber (e.g. 3.1 for 'Create Client' in Client category)"></i>
                                        </span>
                                    </div>
                                </div>
                                <small class="form-text text-muted">Use format: CategoryID.PageNumber (e.g. 3.1 for 'Create
                                    Client' in Client category)</small>
                                @error('code')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control @error('page_category_id') is-invalid @enderror"
                                    id="page_category_id" name="page_category_id" required>
                                    <option value="">Select Category</option>
                                    @foreach ($categories as $id => $name)
                                        <option value="{{ $id }}"
                                            {{ old('page_category_id', $page->page_category_id) == $id ? 'selected' : '' }}>
                                            {{ $name }}</option>
                                    @endforeach
                                </select>
